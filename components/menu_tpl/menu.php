   ...
   use yii\helpers\Url;
    ...
    $idn = $category[id];
	if ($idn == 186) {?> 
	    <a href = "<?= Url::to(['nedvigcategory/8', 'ads' =>$tkva])?>">
		<b> Квартиры </b>
		<?php } elseif ($idn == 179) { ?>
	 <a href = "<?= Url::to(['site/mainned'])?>"><strong> Недвижимость </strong><span class="badge pull-right"><i class="fa fa-bars"></i></span></a>		
	<?php } elseif ($idn == 187) { ?>
	 <a href = "<?= Url::to(['nedvigcategory/2', 'ads' =>$tdom])?>">
		<b> Дома/Коттеджи/Дачи  </b>
		<?php } elseif ($idn == 188) { ?>
	 <a href = "<?= Url::to(['nedvigcategory/3', 'ads' =>$tkom])?>">
		<b> Комнаты  </b>
		<?php } elseif ($idn == 189) { ?>
	 <a href = "<?= Url::to(['nedvigcategory/19', 'ads' =>$tkomm])?>">
		<b> Коммерческая  </b>
		<?php } elseif ($idn == 190) { ?>
	 <a href = "<?= Url::to(['nedvigcategory/14', 'ads' =>$tucha])?>">
		<b> Земельные участки   </b>
		<?php } elseif ($idn == 191) { ?>
	 <a href = "<?= Url::to(['nedvigcategory/6', 'ads' =>$tgarag])?>">
		<b> Гаражи   </b>
		<?php } elseif ($idn == 192) { ?>
	 <a href = "<?= Url::to(['nedvigcategory/7', 'ads' =>$tbiznes])?>">
		<b> Готовый бизнес   </b>
		<?php } elseif ($idn == 180) { ?>
	 <a href = "<?= Url::to(['avtocategory/1', 'ads' =>$aleg])?>">
		<b> Легковые авто  </b>
		<?php } elseif ($idn == 181) { ?>
	 <a href = "<?= Url::to(['avtocategory/2', 'ads' =>$agruz])?>">
		<b> Грузовые//Автобусы </b>
		<?php } elseif ($idn == 178) { ?>
	 <a style = "cursor: pointer" href = "<?= Url::to(['site/mainavto'])?>"><strong> Автотехника </strong><span class="badge pull-right"><i class="fa fa-bars"></i></span></a>		
		<?php } elseif ($idn == 182) { ?>
	 <a href = "<?= Url::to(['avtocategory/3', 'ads' =>$aspec])?>">
		<b> Спецтехника  </b>
		<?php } elseif ($idn == 183) { ?>
	 <a href = "<?= Url::to(['avtocategory/4', 'ads' =>$amoto])?>">
		<b> Мототехника </b>
		<?php } elseif ($idn == 184) { ?>
	 <a href = "<?= Url::to(['avtocategory/5', 'ads' =>$akompl])?>">
		<b> Комплектующие для авто</b>
		<?php } elseif ($idn == 185) { ?>
	 <a href = "<?= Url::to(['avtocategory/6', 'ads' =>$avod])?>">
	 ...
	 <a style = "cursor: pointer"><strong>Все Категории</strong><span class="badge pull-right"><i class="fa fa-bars"></i></span></a>
		<?php } else { ?>
    <a href = "<?= Url::to(['category/'.$category['id'], 'ads' =>$s])?>">
