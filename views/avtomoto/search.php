...
<select id="product-category_id" class="form-control" name="sost[id]">
    ...
    border-radius: 8px;			
                                            ...
                                            <h2 class="title text-center"> Расширенный поиск мототехники
											 ...
											 <span class="letter"><img src="/images/nastroika.png" alt="поиск"></span></a>	
                                                ...
                                                <?php $mainImg = $avtomotos->getImage();  ?>
						<div class="col-sm-3 animated pulse faster">
							<center><?php if(!empty($avtomotos->videl)){ ?> 
								...
								<div class="single-products1">
									<div class="productinfo text-center">
										  ...
										  <?php if ($mainImg['urlAlias']==='placeHolder') { ?>
                                                  ...
                                                  <a href="<?= yii\helpers\Url::to(['avtomoto/'.$avtomotos->id]) ?>"
												  <li><?=Html::img($mainImg->getUrl(''), ['alt'=> $avtomotos->tip]) ?></li></a>
												                  
																			 <?php } else { ?> 
																			 
												  <a href="<?= yii\helpers\Url::to(['avtomoto/'.$avtomotos->id]) ?>"
												  <li><?=Html::img($mainImg->getUrl('335X210'), ['alt'=> $avtomotos->tip]) ?></li></a>
												 
												                             <?php } ?>
										 
				<p><h5><?= $avtomotos -> tip ?></h5></p>
				     <h5><b>год:</b> <?= $avtomotos -> god ?></h5>
					 <h5><b>состояние:</b> <?= $avtomotos -> sostoyanie ?></h5>
										<h5><b>cтоимость:</b> <?= $avtomotos -> price ?> <b>руб.</b></h5>
                        ...
                        <li>КРАТКОЕ ОПИСАНИЕ:<hr> <?= mb_substr($avtomotos->opisanie, 0, 150);  ?><br>
									  <a href="<?= yii\helpers\Url::to(['avtomoto/'.$avtomotos->id]) ?>" class="btn btn-default1">подробнее</a>
						</li>
                            ...
                            <div id="brands_products2">
									...
									<span>КРАТКОЕ ОПИСАНИЕ:<hr><?= mb_substr($avtomotos->opisanie, 0, 230);  ?><br>
									 <a href="tel:<?= $avtomotos -> telephone ?>" target="_blank" class="btn btn-default1"> позвонить</a>
									 <a href="<?= yii\helpers\Url::to(['avtomoto/'.$avtomotos->id]) ?>" class="btn btn-default1">подробнее</a>
									</span></small>
									 </div>
									</div>										  
