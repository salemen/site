...
<div class="brands-name left-sidebar" id="brands_products1">
	   <div id="fixed"> 
        ...
        <center> <h2> поиск гаражей </h2> </center>	
			 ...
			 <form method="get" action="<?= yii\helpers\Url::to(['nedviggaragi/search' ]) ?>">
...
<select id="product-category_id" class="form-control" name="Ochrana[id]">
	...
	border-radius: 8px;	
                                            ...
                                            <h2 class="title text-center"> Расширенный поиск гаражей
											<a href ="<?= yii\helpers\Url::to (['cart/garagi']) ?>" class= "showSearchgaragi">
											 <span class="letter"><img src="/images/nastroika.png" alt="поиск гаражей в Краснодарском крае"></span></a>	
											</h2>
                                              </div>
                       ...
                       $nedviggaragis->raion = $item;}}
						...
						<div class="col-sm-3 animated pulse faster">
							<center> <?php if(!empty($nedviggaragis->videl)){ ?> 
								...
								<div class="single-products1">
									<div class="productinfo text-center">
										  <?php if ($mainImg['urlAlias']==='placeHolder') { ?>
												  ...
												  <li><?=Html::img($mainImg->getUrl(''), ['alt' => $nedviggaragi -> gorod]) ?></li></a>
												                  
																			 <?php } else { ?> 
																			 
												  <a href="<?= yii\helpers\Url::to(['nedviggaragi/'.$nedviggaragis->id,'ads' =>'garag_plochad_'.$nedviggaragis->plochad.'m.kv._price_'.$nedviggaragis->price]) ?>">
												  <li><?=Html::img($mainImg->getUrl('335X210'), ['alt' => $nedviggaragi -> gorod]) ?></li></a>
												 
												                             <?php } ?>
				<p><h5>Гараж в <?= $nedviggaragis->raion ?> район</h5></p> 
										<h5><b>цена:</b> <?= $formatted_number = number_format($nedviggaragis->price, 0, ',', ' '); ?> руб.</h5>
										      
                                         <p><h5><b>Площадью:</b> <?= $nedviggaragis->plochad ?> </b>м.кв.</b></h5></p>
													...
													<p><a href ="<?= yii\helpers\Url::to (['cart/addgaragi', 'id'=> $nedviggaragis->id]) ?>" data-id = "<?= $nedviggaragis->id?>"
                        ...
                        <li> КРАТКОЕ ОПИСАНИЕ:<hr><?= mb_substr($nedviggaragis->opisanie, 0, 150);  ?><br>
									  <a href="<?= yii\helpers\Url::to(['nedviggaragi/'.$nedviggaragis->id,'ads' =>'garag_plochad_'.$nedviggaragis->plochad.'m.kv._price_'.$nedviggaragis->price]) ?>" class="btn btn-default1">подробнее о гараже</a>
						</li>
                          ...
                          <div id="brands_products2">
									...
									<span>КРАТКОЕ ОПИСАНИЕ:<hr><?= mb_substr($nedviggaragis->opisanie, 0, 230);  ?><br>
									 <a href="<?= yii\helpers\Url::to(['nedviggaragi/'.$nedviggaragis->id,'ads' =>'garag_plochad_'.$nedviggaragis->plochad.'m.kv._price_'.$nedviggaragis->price]) ?>" class="btn btn-default1">подробнее о гараже</a>
									 <a href="tel:<?= $nedviggaragis -> telephone ?>" target="_blank" class="btn btn-default1"> позвонить</a>
