...
<select id="product-category_id" class="form-control" name="Operaciya[id]">
    ...
    border-radius: 8px;		
                                            ...
                                            <h2 class="title text-center"> Расширенный поиск комнат 
											 <a href ="<?= Url::to (['cart/komn']) ?>" class= "showSearchkomnati">
											   <span class="letter"><img src="/images/nastroika.png" alt="поиск комнат в Краснодарском крае"></span></a>	
											</h2>
                                               </div>
                       ...
                       $komnatis->raion = $item;}}
						...
						<div class="col-sm-3 animated pulse faster">
							<center><?php if(!empty($komnatis->videl)){ ?> 
								...
								<div class="single-products1">
									<div class="productinfo text-center">
										  <?php if ($mainImg['urlAlias']==='placeHolder') { ?>
                                                  ...
                                                  <a href="<?= Url::to(['nedvigkomnati/'.$komnatis->id,'ads' =>'komnata_plochad_'.$komnatis->plochad.'m.kv._price_'.$komnatis->price]) ?>"> 
												  <li><?=Html::img($mainImg->getUrl(''), ['alt' => 'Купить комнату '. $komnatis -> plochad .' м.кв. цена '.$komnatis -> price.' руб. г.'.$komnatis->gorod]) ?></li></a>
												                  
																			 <?php } else { ?> 
																			 
												  <a href="<?= Url::to(['nedvigkomnati/'.$komnatis->id,'ads' =>'komnata_plochad_'.$komnatis->plochad.'m.kv._price_'.$komnatis->price]) ?>"> 
												  <li><?=Html::img($mainImg->getUrl('335X210'), ['alt' => 'Купить комнату '. $komnatis -> plochad .' м.кв. цена '.$komnatis -> price.' руб. г.'.$komnatis->gorod]) ?></li></a>
												 
												                             <?php } ?>
				<p> <a href="<?= Url::to(['nedvigkomnati/'.$komnatis->id,'ads' =>'komnata_plochad_'.$komnatis->plochad.'m.kv._price_'.$komnatis->price]) ?>"> 
				<h5><?= $komnatis->operaciya ?> комнаты</h5></a></p>
										<h5><b>цена:</b> <?= $formatted_number = number_format($komnatis->price, 0, ',', ' '); ?> руб.</h5>
										
										
                                                <p><h5><b>Район:</b> <?= $komnatis->raion ?></h5></p> 
                                         <p><h5><b>Площадь: </b><?= $komnatis->plochad ?> м.кв.</h5></p>
                                                    ...
                                                    <?php if ($komnatis->sale):?>
                                                                    <?= Html::img("@web/images/home/sale.png", ['alt'=> 'Скидка', 'class' => 'sale' ]) ?>
                                                                    <?php endif ?>
																	
													 <p><a href ="<?= Url::to (['cart/addkomnati', 'id'=> $komnatis->id]) ?>" data-id = "<?= $komnatis->id?>"
                        ...
                        <li> КРАТКОЕ ОПИСАНИЕ:<hr> <?= mb_substr($komnatis->opisanie, 0, 150);  ?><br>
									  <a href="<?= Url::to(['nedvigkomnati/'.$komnatis->id,'ads' =>'komnata_plochad_'.$komnatis->plochad.'m.kv._price_'.$komnatis->price]) ?>" class="btn btn-default1">подробнее о комнате</a>
						</li>
						 ...
						 <div id="brands_products2">
									...
									<span>КРАТКОЕ ОПИСАНИЕ:<hr><?= mb_substr($komnatis->opisanie, 0, 230);  ?><br>
									 <a href="<?= Url::to(['nedvigkomnati/'.$komnatis->id,'ads' =>'komnata_plochad_'.$komnatis->plochad.'m.kv._price_'.$komnatis->price]) ?>" class="btn btn-default1">подробнее о комнате</a>
									 <a href="tel:<?= $komnatis -> telephone ?>" target="_blank" class="btn btn-default1"> позвонить</a>
