...
class NedvigzemliController extends Controller
{
   ...
   public function actionIndex(){
    ...
    public function actionView($id)
    {
		$username = Yii::$app->user->identity['username'];
             ...
             Yii::$app->mailer->compose('ads', ['username' => $username] )->setFrom (['rim-79@bk.ru'=>'Новое объявление на сайте'])
			  -> setTo('info@salemen.ru')-> setSubject('Новое объявление, ЗЕМЕЛЬНЫЕ УЧАСТКИ')->send ();}
    ...
    public function actionCreate()
    {
       ...
       // формирование подписи
	    $crc  = md5("$mrh_login:$out_summ:$inv_id:$mrh_pass1:Shp_id=$id_id:Shp_item=$shp_item:Shp_pricehit=$price_hit:Shp_pricenew=$price_new:Shp_pricerekom=$price_rekom:Shp_pricesale=$price_sale:Shp_priceverh=$price_verh:Shp_pricevidel=$price_videl:Shp_username=$username"); 			
            ...
            unset($model->image);
             ...
             if ($oplata > 0){
		    Yii::$app->session->setFlash('success', "<center>Участок добавлен на сайт. Для применения опций к объявлению, ждем оплаты.</center>");
	   ...
	   Yii::$app->session->setFlash('success', "<center>Участок добавлен на сайт. После модерации появится в Базе Данных</center>");
	   return $this->redirect(['view', 'id' => $model->id]);		
