...
/**
 * NedviggaragiController implements the CRUD actions for NedvigGaragi model.
 */
class NedviggaragiController extends Controller
{
    /**
     * {@inheritdoc}
     */
       ...
       
    {
     $username = Yii::$app->user->identity['username'];
        if ($username=='sarmetradmin'){
         $query1 = Nedviggaragi::find()->select($id)->where(['moder' => 0])->one();		 
		 if (!empty($query1)){
			 $query = Nedviggaragi::find()->select($id)->orderBy(['moder' => 0]); 
         }elseif(empty($query1)){
			 $query = Nedviggaragi::find()->select($id)->orderBy(['id' => SORT_DESC]); 
		 } 
		 $pages = new Pagination (['totalCount'=> $query->count(), 'pageSize' => 24, 'forcePageParam'=> false, 'pageSizeParam'=> false ]);
          $Nedviggaragi = $query->offset($pages->offset)->limit($pages->limit)->all();
              ...
              $query = Nedviggaragi::find()->select($id)->orderBy(['id' => SORT_DESC])->where(['username'=>$username]);
              $pages = new Pagination (['totalCount'=> $query->count(), 'pageSize' => 24, 'forcePageParam'=> false, 'pageSizeParam'=> false ]);
              $Nedviggaragi = $query->offset($pages->offset)->limit($pages->limit)->all();
              
        }
              
      return $this ->render('index',compact ('Nedviggaragi','pages'),[
    ...
    /**
     * Displays a single NedvigGaragi model.
     * @param integer $id
     * @return mixed
    ...
    public function actionView($id)
    {
		$username = Yii::$app->user->identity['username'];
             ...
             Yii::$app->mailer->compose('ads', ['username' => $username] )->setFrom (['rim-79@bk.ru'=>'Новое объявление на сайте'])
			  -> setTo('info@salemen.ru')-> setSubject('Новое объявление, ГАРАЖИ')->send ();}
    ...
    /**
     * Creates a new NedvigGaragi model.
    ...
    /**
     * Updates an existing NedvigGaragi model.
    ...
    public function actionUpdate($id)
   {
    ...
    /**
     * Deletes an existing NedvigGaragi model.
    ...
    /**
     * Finds the NedvigGaragi model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return NedvigGaragi the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = NedvigGaragi::findOne($id)) !== null) {
            return $model;
        }
