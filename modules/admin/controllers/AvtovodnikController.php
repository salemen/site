...
/**
 * AvtovodnikController implements the CRUD actions for Avtovodnik model.
 */
class AvtovodnikController extends Controller
{
    /**
     * {@inheritdoc}
     */
       ...
       
    {
      $username = Yii::$app->user->identity['username'];
        if ($username=='sarmetradmin'){
         $query1 = Avtovodnik::find()->select($id)->where(['moder' => 0])->one();		 
		 if (!empty($query1)){
			 $query = Avtovodnik::find()->select($id)->orderBy(['moder' => 0]); 
         }elseif(empty($query1)){
			 $query = Avtovodnik::find()->select($id)->orderBy(['id' => SORT_DESC]); 
		 } 
		 $pages = new Pagination (['totalCount'=> $query->count(), 'pageSize' => 24, 'forcePageParam'=> false, 'pageSizeParam'=> false ]);
          $Avtovodnik = $query->offset($pages->offset)->limit($pages->limit)->all();
              ...
              $query = Avtovodnik::find()->select($id)->orderBy(['id' => SORT_DESC])->where(['username'=>$username]);
              $pages = new Pagination (['totalCount'=> $query->count(), 'pageSize' => 24, 'forcePageParam'=> false, 'pageSizeParam'=> false ]);
              $Avtovodnik = $query->offset($pages->offset)->limit($pages->limit)->all();
              
        }
              
      return $this ->render('index',compact ('Avtovodnik','pages'),[
    ...
    /**
     * Displays a single Avtovodnik model.
     * @param integer $id
     * @return mixed
    ...
    public function actionView($id)
    {
		$username = Yii::$app->user->identity['username'];
             ...
             Yii::$app->mailer->compose('ads', ['username' => $username] )->setFrom (['rim-79@bk.ru'=>'Новое объявление'])
			  -> setTo('info@salemen.ru')-> setSubject('Новое объявление, ВОДНЫЙ ТРАНСПОРТ')->send ();}
    ...
    /**
     * Creates a new Avtovodnik model.
    ...
    /**
     * Updates an existing Avtovodnik model.
    ...
    /**
     * Deletes an existing Avtovodnik model.
    ...
    /**
     * Finds the Avtovodnik model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Avtovodnik the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Avtovodnik::findOne($id)) !== null) {
            return $model;
        }
