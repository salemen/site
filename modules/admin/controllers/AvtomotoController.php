...
/**
 * AvtomotoController implements the CRUD actions for Avtomoto model.
 */
class AvtomotoController extends Controller
{
    /**
     * {@inheritdoc}
     */
   ...
   public function actionIndex(){
       ...
       
    {
      $username = Yii::$app->user->identity['username'];
        if ($username=='sarmetradmin'){
         $query1 = Avtomoto::find()->select($id)->where(['moder' => 0])->one();		 
		 if (!empty($query1)){
			 $query = Avtomoto::find()->select($id)->orderBy(['moder' => 0]); 
         }elseif(empty($query1)){
			 $query = Avtomoto::find()->select($id)->orderBy(['id' => SORT_DESC]); 
		 } 
		 $pages = new Pagination (['totalCount'=> $query->count(), 'pageSize' => 24, 'forcePageParam'=> false, 'pageSizeParam'=> false ]);
          $Avtomoto = $query->offset($pages->offset)->limit($pages->limit)->all();
              ...
              $query = Avtomoto::find()->select($id)->orderBy(['id' => SORT_DESC])->where(['username'=>$username]);
              $pages = new Pagination (['totalCount'=> $query->count(), 'pageSize' => 24, 'forcePageParam'=> false, 'pageSizeParam'=> false ]);
              $Avtomoto = $query->offset($pages->offset)->limit($pages->limit)->all();
              
        }
              
      return $this ->render('index',compact ('Avtomoto','pages'),[
    ...
    /**
     * Displays a single Avtomoto model.
     * @param integer $id
     * @return mixed
    ...
    public function actionView($id)
    {
		$username = Yii::$app->user->identity['username'];
             ...
             Yii::$app->mailer->compose('ads', ['username' => $username] )->setFrom (['rim-79@bk.ru'=>'Новое объявление на сайте'])
			  -> setTo('info@salemen.ru')-> setSubject('Новое объявление, МОТОТЕХНИКА')->send ();}
    ...
    /**
     * Updates an existing Avtomoto model.
    ...
    /**
     * Deletes an existing Avtomoto model.
    ...
    /**
     * Finds the Avtomoto model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Avtomoto the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Avtomoto::findOne($id)) !== null) {
            return $model;
        }
