...
/**
 * NedvigbiznesController implements the CRUD actions for Nedvigbiznes model.
 */
class NedvigbiznesController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    {   $id = Yii::$app->request->get('id');
      ...
      $username = Yii::$app->user->identity['username'];
        if ($username=='sarmetradmin'){
         $query1 = Nedvigbiznes::find()->select($id)->where(['moder' => 0])->one();		 
		 if (!empty($query1)){
			 $query = Nedvigbiznes::find()->select($id)->orderBy(['moder' => 0]); 
         }elseif(empty($query1)){
			 $query = Nedvigbiznes::find()->select($id)->orderBy(['id' => SORT_DESC]); 
		 } 
		 $pages = new Pagination (['totalCount'=> $query->count(), 'pageSize' => 24, 'forcePageParam'=> false, 'pageSizeParam'=> false ]);
          $Nedvigbiznes = $query->offset($pages->offset)->limit($pages->limit)->all();
              ...
              $query = Nedvigbiznes::find()->select($id)->orderBy(['id' => SORT_DESC])->where(['username'=>$username]);
              $pages = new Pagination (['totalCount'=> $query->count(), 'pageSize' => 24, 'forcePageParam'=> false, 'pageSizeParam'=> false ]);
              $Nedvigbiznes = $query->offset($pages->offset)->limit($pages->limit)->all();
              
        }
              
      return $this ->render('index',compact ('Nedvigbiznes','pages'),[
    ...
    /**
     * Displays a single Nedvigbiznes model.
     * @param integer $id
     * @return mixed
    ...
    public function actionView($id)
    {
		$username = Yii::$app->user->identity['username'];
             ...
             Yii::$app->mailer->compose('ads', ['username' => $username] )->setFrom (['rim-79@bk.ru'=>'Новое объявление на сайте'])
			  -> setTo('info@salemen.ru')-> setSubject('Новое объявление, БИЗНЕС')->send ();}
    ...
    /**
     * Creates a new Nedvigbiznes model.
    ...
    public function actionCreate()
    {
	    ...
	    $crc  = md5("$mrh_login:$out_summ:$inv_id:$mrh_pass1:Shp_id=$id_id:Shp_item=$shp_item:Shp_pricehit=$price_hit:Shp_pricenew=$price_new:Shp_pricerekom=$price_rekom:Shp_pricesale=$price_sale:Shp_priceverh=$price_verh:Shp_pricevidel=$price_videl:Shp_username=$username"); 			
          ...
          $model->image = UploadedFile::getInstance($model, 'image');
            ...
            unset($model->image);
             ...
             if ($oplata > 0){
		    Yii::$app->session->setFlash('success', "<center>Бизнес добавлен на сайт. Для применения опций к объявлению, ждем оплаты.</center>");
	   ...
	   Yii::$app->session->setFlash('success', "<center>Бизнес добавлен на сайт. После модерации появится в Базе Данных</center>");
	   return $this->redirect(['view', 'id' => $model->id]);		
    ...
    /**
     * Updates an existing Nedvigbiznes model.
    ...
    /**
     * Deletes an existing Nedvigbiznes model.
    ...
    /**
     * Finds the Nedvigbiznes model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Nedvigbiznes the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Nedvigbiznes::findOne($id)) !== null) {
            return $model;
        }
