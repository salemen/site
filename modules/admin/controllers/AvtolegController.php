...
/**
 * AvtolegController implements the CRUD actions for Avtoleg model.
 */
class AvtolegController extends Controller
{
    /**
     * {@inheritdoc}
     */
       ...
       
    {
      $username = Yii::$app->user->identity['username'];
        if ($username=='sarmetradmin'){
         $query1 = Avtoleg::find()->select($id)->where(['moder' => 0])->one();		 
		 if (!empty($query1)){
			 $query = Avtoleg::find()->select($id)->orderBy(['moder' => 0]); 
         }elseif(empty($query1)){
			 $query = Avtoleg::find()->select($id)->orderBy(['id' => SORT_DESC]); 
		 } 
		 $pages = new Pagination (['totalCount'=> $query->count(), 'pageSize' => 24, 'forcePageParam'=> false, 'pageSizeParam'=> false ]);
          $Avtoleg = $query->offset($pages->offset)->limit($pages->limit)->all();
              ...
              $query = Avtoleg::find()->select($id)->orderBy(['id' => SORT_DESC])->where(['username'=>$username]);
              $pages = new Pagination (['totalCount'=> $query->count(), 'pageSize' => 24, 'forcePageParam'=> false, 'pageSizeParam'=> false ]);
              $Avtoleg = $query->offset($pages->offset)->limit($pages->limit)->all();
              
        }
              
      return $this ->render('index',compact ('Avtoleg','pages'),[
    ...
    /**
     * Displays a single Avtoleg model.
     * @param integer $id
     * @return mixed
    ...
    public function actionView($id)
    {
		$username = Yii::$app->user->identity['username'];
             ...
             Yii::$app->mailer->compose('ads', ['username' => $username] )->setFrom (['rim-79@bk.ru'=>'Новое объявление на сайте'])
			  -> setTo('info@salemen.ru')-> setSubject('Новое объявление, ЛЕГКОВЫЕ АВТО')->send ();}
    ...
    /**
     * Updates an existing Avtoleg model.
    ...
    /**
     * Deletes an existing Avtoleg model.
    ...
    /**
     * Finds the Avtoleg model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Avtoleg the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Avtoleg::findOne($id)) !== null) {
            return $model;
        }
