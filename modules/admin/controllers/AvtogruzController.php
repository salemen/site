...
/**
 * AvtogruzController implements the CRUD actions for Avtogruz model.
 */
class AvtogruzController extends Controller
{
    /**
     * {@inheritdoc}
     */
...
//        }
    {
      $username = Yii::$app->user->identity['username'];
        if ($username=='sarmetradmin'){
         $query1 = Avtogruz::find()->select($id)->where(['moder' => 0])->one();		 
		 if (!empty($query1)){
			 $query = Avtogruz::find()->select($id)->orderBy(['moder' => 0]); 
         }elseif(empty($query1)){
			 $query = Avtogruz::find()->select($id)->orderBy(['id' => SORT_DESC]); 
		 } 
		 $pages = new Pagination (['totalCount'=> $query->count(), 'pageSize' => 24, 'forcePageParam'=> false, 'pageSizeParam'=> false ]);
          $Avtogruz = $query->offset($pages->offset)->limit($pages->limit)->all();
              ...
              $query = Avtogruz::find()->select($id)->orderBy(['id' => SORT_DESC])->where(['username'=>$username]);
              $pages = new Pagination (['totalCount'=> $query->count(), 'pageSize' => 24, 'forcePageParam'=> false, 'pageSizeParam'=> false ]);
              $Avtogruz = $query->offset($pages->offset)->limit($pages->limit)->all();
              
        }
              
      return $this ->render('index',compact ('Avtogruz','pages'),[
            
        ]);
    }
    }
    /**
     * Displays a single Avtogruz model.
     * @param integer $id
     * @return mixed
    ...
    public function actionView($id)
    {
		 $username = Yii::$app->user->identity['username'];
             ...
             Yii::$app->mailer->compose('ads', ['username' => $username] )->setFrom (['rim-79@bk.ru'=>'Новое объявление на сайте'])
			  -> setTo('info@salemen.ru')-> setSubject('Новое объявление, ГРУЗОВОЙ ТРАНСПОРТ')->send ();}
    ...
    /**
     * Updates an existing Avtogruz model.
    ...
    /**
     * Deletes an existing Avtogruz model.
    ...
    /**
     * Finds the Avtogruz model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Avtogruz the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Avtogruz::findOne($id)) !== null) {
            return $model;
        }
