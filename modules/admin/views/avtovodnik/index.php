...
$this->title = 'Мой Водный транспорт';
$this->params['breadcrumbs'][] = $this->title;
?>
<div class="avtovodnik-index">
        ...
        <center> 
         ...
         <a href="<?= Url::to(['avtovodnik/index']) ?>" class="btn btn-danger">Водный транспорт</a>
...
<table>
    <center> <h4>Водный транспорт: мои объявления</h4></center>
                 ...
                 $mainImg = $Avtovodniks->getImage();
						...
						<?php } ?>
								<div class="single-Products">
                                       ...
                                       <?php if (!$Avtovodniks->moder) { 
									echo '<style="color: red">На модерации</style>';
									}?>
                                                                                   
                                              <?php if ($mainImg['urlAlias']==='placeHolder') { ?>
																			 
											 <a href="<?= Url::to(['/admin/avtovodnik/view', 'id'=>$Avtovodniks->id]) ?>"		 
											 ...
											 <a href="<?= Url::to(['/admin/avtovodnik/view', 'id'=>$Avtovodniks->id]) ?>"								 
												 <li><?=Html::img($mainImg->getUrl('x200'), ['alt' => '']) ?></li></a>
												                             ...
												                             <?php } ?>  
                                                 <h5> ID <?= $Avtovodniks->id ?>	</h5>																			 
											
                                                                      <h5><?= $Avtovodniks -> tip ?></h5>
                                                                     <h5><?= $Avtovodniks->price ?> руб.</h5> 
																	 
                            <!-- <a href="<?= Url::to(['/admin/avtovodnik/view', 'id'=>$Avtovodniks->id]) ?>" class="btn btn-default">подробнее</a>-->
                             <a href="<?= Url::to(['/admin/avtovodnik/update', 'id'=>$Avtovodniks->id]) ?>" class="btn btn-default">изменить и продать быстрее</a></br>
                         
                             <?php echo Html::a('Удалить', ['/admin/avtovodnik/delete', 'id'=>$Avtovodniks->id], [
                                                           'data' => [
                                                           'method' => 'post',
