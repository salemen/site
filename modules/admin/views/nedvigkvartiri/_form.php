...
<div class="nedvig-kvartiri-form">
    ...
    <?php $form = ActiveForm::begin(['options' => ['enctype' => 'multipart/form-data']]); ?>
     ...
     <?php
//   echo $form->field($model, 'opisanie')->widget(CKEditor::className(), [
//  
  ...
  <style>
   ...
   <tr id="thisguys" class="noshowtrue">
	<td align="left" colspan="2">
		<center><p><strong>Укажите местоположение квартиры на карте</strong></p></center>
		...
		<script type="text/javascript">
			...
			$.ajax({
        type: 'POST',
        url: 'nedvigkvartiri/update', // урл вставьте свой, этот для примера
        data: {
            declat:declat,
            declong:declong,
	...
	&nbsp;<center><a href="javascript:void(0)" onclick="hidethisguys()"><i class="btn btn-default" 
	       style= "margin-left: 5px; margin-top: 5px;" ><i class="fa fa-flag"></i> скрыть карту </i></a></center>
	</td>
</tr>
					</table>	
...
//debug($id_oblast)
  ?>
	...
	<td><?= $form->field($model, 'street')->textInput(['placeholder' => 'обязательное поле']) ?></td>
    
</tr>
<tr>
    <td><?= $form->field($model, 'number_doma')->textInput(['placeholder' => 'желательное поле']) ?></td>
	<td><?= $form->field($model, 'number_kvartir')->textInput(['type' => 'number','placeholder' => 'желательное поле']) ?></td>    
</tr>

<tr>
   <td><?= $form->field($model, 'kolichestvo_komnat')->dropDownList( kolichestvokomnat::find()->select(['number','id'])->indexBy('number')->column(),['promt'=>'Select kolichestvokomnat',]) ?></td> 
   <td><?= $form->field($model, 'vtorichka_novostroi')->dropDownList( vidnedvig::find()->select(['vid','id'])->indexBy('vid')->column(),['promt'=>'Select vid']) ?></td>     
</tr>

<tr>
   
    <td><?= $form->field($model, 'operaciya')->dropDownList( operaciya::find()->select('tip')->indexBy('tip')->column(),['promt'=>'Select operaciya']) ?></td>  
    <td><?= $form->field($model, 'tip_doma')->dropDownList( tipdoma::find()->select(['tip','id'])->indexBy('tip')->column(),['promt'=>'Select tip']) ?></td>
</tr>


<tr>

    <td><?= $form->field($model, 'etag')->dropDownList( etag::find()->select(['etag','id'])->indexBy('etag')->column(),[
'options' => [
'1' => ['selected' => true]
]]) ?></td>
    <td><?= $form->field($model, 'etagey_v_dome')->dropDownList( etagei::find()->select(['etagei','id'])->indexBy('etagei')->column(),['promt'=>'Select etagei']) ?></td>
</tr>

<tr>
    <td><?= $form->field($model, 'plochad_obchy')->textInput(['type' => 'number','placeholder' => 'обязательное поле']) ?></td>
	
	<td><?= $form->field($model, 'plochad_gilaya')->textInput(['type' => 'number','placeholder' => 'желательное поле']) ?></td>
</tr>

<tr>
    <td><?= $form->field($model, 'plochad_kuxni')->textInput(['type' => 'number','placeholder' => 'желательное поле']) ?></td>
...
<center> <div id="fff"><table class="table table-hover table-striped"> 
 ...
 <?php if(!empty($model->verh)){?>
 <tr><td><label>
<?= $form->field($model, 'verh')->checkbox(['label' => 'Уже <span style="color: blue;">ЗАКРЕПЛЕНО</span> на верху', 'checked ' => true]) ?>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;( <?=$prverh;?> руб. )<small class="ttip">ЧТО ЭТО ?<span><?=$a2zz;?></span></small>
</label></td></tr> 
 <?php } elseif(!$model->verh) { ?>
<tr><td><label>
<?= $form->field($model, 'verh')->checkbox(['label' => 'Закрепить <span style="color: blue;">НА ВЕРХУ</span> в Квартирах']) ?>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;( <?=$prverh;?> руб. )<small class="ttip">ЧТО ЭТО ?<span><?=$a2zz;?></span></small>
</label></td></tr>
 <?php }?>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;( <?=$prhit;?> руб. )<small class="ttip">ЧТО ЭТО ?<span><?=$ab1dd;?></span></small>
</label></td></tr> 
 <?php } elseif(!$model->hit) { ?>
<tr><td><label>
<?= $form->field($model, 'hit')->checkbox(['label' => 'Добавить в <span style="color: blue;">ИЗБРАННЫЕ</span> Квартиры']) ?>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;( <?=$prhit;?> руб. )<small class="ttip">ЧТО ЭТО ?<span><?=$ab1dd;?></span></small>
</label></td></tr>
 <?php }?>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;( <?=$prrekom;?> руб. )<small class="ttip">ЧТО ЭТО ?<span><?=$ab4dd;?></span></small>
</label></td></tr>
...
<?php }?>
 ...
 <?= $form->field($model, 'moder')->checkbox([ '0', '1' ]) ?><?php } ?>
    ...
    <center><div class="form-group"><?php if ($model->isNewRecord) {?><?php } ?><br>
