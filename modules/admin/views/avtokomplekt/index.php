...
$this->title = ' Мои объявления, Комплектующие';
$this->params['breadcrumbs'][] = $this->title;
?>
<div class="avtokomplekt-index">
        ...
        <center> 
         ...
         <a href="<?= Url::to(['avtokomplekt/index']) ?>" class="btn btn-danger">Комплектующие и автозапчасти</a>
...
<table>
    <center> <h4>Комплектующие и акссесуары: мои объявления</h4> </center>
...
.btn-1success {
	...
	<div class="features_items"><!--features_items-->
                                            ...
                                            <?php foreach ($Avtokomplekt as $Avtokomplekts):?>
                                              ...
                                              <?php
                                $mainImg = $Avtokomplekts->getImage();
                           
						     $model2 = Avtocategory::find()->select('name')
								->where(['id' => $Avtokomplekts -> tip])
								->one();      
								   $Avtokomplekts -> tip = $model2[name];    
                                               ?>
						...
						<?php } ?>
								<div class="single-Products">
                                       ...
                                       <?php if (!$Avtokomplekts->moder) { 
									echo '<style="color: red">На модерации</style>';
									}?>
                                                                                   
                                              <?php if ($mainImg['urlAlias']==='placeHolder') { ?>
																			 
											 <a href="<?= Url::to(['/admin/avtokomplekt/view', 'id'=>$Avtokomplekts->id]) ?>"		 
											 ...
											 <a href="<?= Url::to(['/admin/avtokomplekt/view', 'id'=>$Avtokomplekts->id]) ?>"								 
												 <li><?=Html::img($mainImg->getUrl('x200'), ['alt' => '']) ?></li></a>
												                             ...
												                             <?php } ?>  
                                           <h5> ID <?= $Avtokomplekts->id ?><p><?= $Avtokomplekts -> tip ?></p>		</h5>																	 
											<h5><?= $Avtokomplekts->price ?> руб.</h5>
                                                                                       
                                                                                       
                             <!--<a href="<?= Url::to(['/admin/avtokomplekt/view', 'id'=>$Avtokomplekts->id]) ?>" class="btn btn-default">подробнее</a>-->
                             <a href="<?= Url::to(['/admin/avtokomplekt/update', 'id'=>$Avtokomplekts->id]) ?>" class="btn btn-default">изменить и продать быстрее</a></br>
                         
                             <?php echo Html::a('Удалить', ['/admin/avtokomplekt/delete', 'id'=>$Avtokomplekts->id], [
                                                           'data' => [
                                                           'method' => 'post',
