...
<style>
 ...
 .nedvig-kommercheska-form div{
	 max-width: 800px;
 }
</style>

<div class="nedvig-kommercheska-form">
   ...
   <!--Ваш логин -->
    ...
    <?php 
		        $username = Yii::$app->user->identity['username'];
				 $idn = $_GET['id'];
				 if ($idn){
				 $username1= nedvigkommercheska::find()->where(['id' => $idn])->andWhere(['username' => $username])->all();}?>
  ...
  <style>
   ...
   <tr id="thisguys" class="noshowtrue">
	<td align="left" colspan="2">
		<center><p><strong>Укажите местоположение на карте</strong></p></center>
		...
		<script type="text/javascript">
			...
			$.ajax({
        type: 'POST',
        url: 'nedvigkommercheska/update', // урл вставьте свой, этот для примера
        data: {
            declat:declat,
            declong:declong,
   ...
   <table class="table table-hover table-striped">  
    ...
    <?= $form->field($model, 'adress')->textInput(['placeholder' => 'обязательное поле']) ?>                
...
//debug($id_oblast)
 ...
 <td> <?= $form->field($model, 'operaciya')->dropDownList( operaciya::find()->select(['tip','id'])->indexBy('tip')->column(),['promt'=>'Select operaciya']) ?></td>
    ...
    <tr>
    <td><?= $form->field($model, 'type_nedvigimosty')->dropDownList( kommtip::find()->select(['tip','id'])->indexBy('tip')->column(),['promt'=>'Select operaciya']) ?></td>
    
    <td> <?= $form->field($model, 'nazvanie_obiekta')->textInput(['placeholder' => 'обязательное поле']) ?></td>
</tr>  
<tr>
    <td><?= $form->field($model, 'plochad')->textInput(['type' => 'number','placeholder' => 'обязательное поле, м.кв.']) ?></td>

    <td><?= $form->field($model, 'plochad_uchastka')->textInput(['placeholder' => 'желательное поле, сот.']) ?></td>
</tr>   
<tr>
    <!-- <td> $form->field($model, 'etag')->dropDownList( etag::find()->select(['etag','id'])->indexBy('etag')->column(),['promt'=>'Select etag']) </td> -->	
	<td> <?= $form->field($model, 'kolichestvo_etagei')->dropDownList( etagei::find()->select(['etagei','id'])->indexBy('etagei')->column(),['promt'=>'Select etagei']) ?></td>
	<td><?= $form->field($model, 'price')->textInput(['type' => 'number','placeholder' => 'обязательное поле, руб.']) ?></td>
</tr>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;( <?=$prverh;?> руб. )<small class="ttip">ЧТО ЭТО ?<span><?=$a4zz;?></span></small>
</label></td></tr> 
 <?php } elseif(!$model->verh) { ?>
<tr><td><label>
<?= $form->field($model, 'verh')->checkbox(['label' => 'Закрепить <span style="color: blue;">НА ВЕРХУ</span> в Коммерческой']) ?>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;( <?=$prverh;?> руб. )<small class="ttip">ЧТО ЭТО ?<span><?=$a4zz;?></span></small>
</label></td></tr>
 <?php }?>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;( <?=$prhit;?> руб. )<small class="ttip">ЧТО ЭТО ?<span><?=$ae1dd;?></span></small>
</label></td></tr> 
 <?php } elseif(!$model->hit) { ?>
<tr><td><label>
<?= $form->field($model, 'hit')->checkbox(['label' => 'Добавить в <span style="color: blue;">ИЗБРАННОЕ</span> Коммерческое']) ?>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;( <?=$prhit;?> руб. )<small class="ttip">ЧТО ЭТО ?<span><?=$ae1dd;?></span></small>
</label></td></tr>
 <?php }?>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;( <?=$prrekom;?> руб. )<small class="ttip">ЧТО ЭТО ?<span><?=$ae4dd;?></span></small>
</label></td></tr>
