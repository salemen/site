...
use app\modules\admin\models\Dometagei;
use app\modules\admin\models\Dommaterial;
use app\modules\admin\models\Domrasstoyanie;
...
<style>
 ...
 .nedvig-doma-form div{
	 max-width: 800px;
 }
</style>
<div class="nedvig-doma-form">
 ...
 <!--Ваш логин -->
    ...
    <center><div><table class="table table-hover table-striped">
      ...
      <td colspan="2"><?= $form->field($model, 'opisanie')->textarea(['rows' => 6,'placeholder' => 'обязательное поле, максимум 1500 символов']) ?></td>
  ...
  <style>
   ...
   <tr id="thisguys" class="noshowtrue">
	<td align="left" colspan="2">
		<center><p><strong>Укажите местоположение дома на карте</strong></p></center>
...
//debug($id_oblast)
  ?>
   ...
   <!--$form->field($model, 'kadastrovi_nomer')->textInput(['placeholder' => 'желательное, но не обязательное поле'])-->
</tr>
<tr>
   <td> <?= $form->field($model, 'vid_obiekta')->dropDownList( domvid::find()->select(['vid','id'])->indexBy('vid')->column(),['promt'=>'Select domvid']) ?></td>


    <td><?= $form->field($model, 'etagei_v_dome')->dropDownList( dometagei::find()->select(['etagei','id'])->indexBy('etagei')->column(),['promt'=>'Select dometagei']) ?></td>
</tr>
<tr>
    <td><?= $form->field($model, 'material_sten')->dropDownList( dommaterial::find()->select(['material','id'])->indexBy('material')->column(),['promt'=>'Select dommaterial']) ?></td>

    <td><?= $form->field($model, 'rasstoyanie_do_goroda')->dropDownList( domrasstoyanie::find()->select(['rasstoyanie','id'])->indexBy('rasstoyanie')->column(),['promt'=>'Select rasstoyanie']) ?></td>
</tr>
<tr>
   <td> <?= $form->field($model, 'plochad_doma')->textInput(['type' => 'number','placeholder' => 'обязательное поле']) ?></td>

    <td><?= $form->field($model, 'plochad_uchastka')->textInput(['placeholder' => 'желательное поле']) ?><td/>
</tr>
<tr>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;( <?=$prverh;?> руб. )<small class="ttip">ЧТО ЭТО ?<span><?=$a3zz;?></span></small>
</label></td></tr> 
 <?php } elseif(!$model->verh) { ?>
<tr><td><label>
<?= $form->field($model, 'verh')->checkbox(['label' => 'Закрепить <span style="color: blue;">НА ВЕРХУ</span> в Домах/...']) ?>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;( <?=$prverh;?> руб. )<small class="ttip">ЧТО ЭТО ?<span><?=$a3zz;?></span></small>
</label></td></tr>
 <?php }?>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;( <?=$prhit;?> руб. )<small class="ttip">ЧТО ЭТО ?<span><?=$aс1dd;?></span></small>
</label></td></tr> 
 <?php } elseif(!$model->hit) { ?>
<tr><td><label>
<?= $form->field($model, 'hit')->checkbox(['label' => 'Добавить в <span style="color: blue;">ИЗБРАННЫЕ</span> Дома/...']) ?>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;( <?=$prhit;?> руб. )<small class="ttip">ЧТО ЭТО ?<span><?=$aс1dd;?></span></small>
</label></td></tr>
 <?php }?>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;( <?=$prrekom;?> руб. )<small class="ttip">ЧТО ЭТО ?<span><?=$ac4dd;?></span></small>
</label></td></tr>
